...
{
    public class FireFlowerSprite : AbstractSprite
    {
        public FireFlowerSprite(Texture2D texture, int rows, int columns, int totalFrames, int startingPointX,
            int startingPointY)
        {
            Texture = texture;
            isVisible = true;
        ...
        public override void Draw(SpriteBatch spriteBatch, Vector2 location)
        {
            if (isVisible)
            {
                ...
                Rectangle sourceRectangle = new Rectangle(frameOrigin.X + currentFramePoint.X * frameSize.X,
                    ...
                    frameSize.X,
                    frameSize.Y);
                Rectangle destinationRectangle = new Rectangle(50, 100, width * 2, height * 2);
